...
function cartTotalPrice(camera, subtotal) {
  ...
  document.getElementById("totalPrice")
          .textContent = formatter.format(cartPrice);
...
function deleteCamera(id) {
  ...
  if (camera.quantity > 1) {
    //si la quantité de produit est superieur à 1 la quantité diminue de 1
    camera.quantity--;
  } else {
...
document.querySelectorAll(".deleteCamera")
        .forEach((deleteButton) => {
  // appel de la fonction au clic
  deleteButton.addEventListener("click", () =>
    deleteCamera(deleteButton.dataset.id)
  );
});
...
document.getElementById("clearCart")
  ...
  // au clic sur le bouton vider le panier
...
cartContainer.appendChild(cartForm)
             .innerHTML += `<h2>Veuillez renseigner vos coordonnées pour confirmer votre commande</h2>
...
form.addEventListener("submit", function (e) {
  ...
  fetch(`http://localhost:3000/api/cameras/order`, {
    ...
    mode: "cors",
    body: JSON.stringify({
      //on assigne à la requête les données attendues par le serveur
      contact: {
